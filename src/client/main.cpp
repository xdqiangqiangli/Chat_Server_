...
unordered_map<string, string> command_map = {
    {"help", "show all help"},
    {"chat","ont to ont chat, friendid:message"},
    {"addfriend","add friend, addfriend:friendid"},
    {"creategroup", "create group, creategroup:groupname:groupdesc"},
    {"addgroup", "add group, addgroup:groupid"},
    {"groupchat", "group chat, groupchat:groupid:message"},
    {"loginout", "login out, loginout"}};
...
void ShowCurrentUserData()
{
    ...
    if (!g_current_group_list.empty())
    {
        for (Group &group : g_current_group_list)
        {
            ...
            for (auto &group_user : group.getGroupUser())
            {
